...
2. 打开仓库内的 `ChatGPT Universal Exporter Plus.user.js`，点击 Raw 后安装。
...
- `src/main.js`：开发入口。
- `src/app.js`：导出器主体逻辑。
- `src/config/`、`src/core/`、`src/export/`、`src/ui/`：配置、运行时辅助、ZIP 导出与界面样式模块。
- `scripts/build.mjs`：使用 esbuild 生成可安装的 `ChatGPT Universal Exporter Plus.user.js`。
- `scripts/check.mjs`：重新构建并检查源码与产物语法。
...
## 目录说明
- `ChatGPT Universal Exporter Plus.user.js`：构建后的可安装用户脚本。
- `src/`：ESM 开发源码。
- `scripts/`：构建与检查脚本。
- `legacy/`：历史版本与实验脚本。
